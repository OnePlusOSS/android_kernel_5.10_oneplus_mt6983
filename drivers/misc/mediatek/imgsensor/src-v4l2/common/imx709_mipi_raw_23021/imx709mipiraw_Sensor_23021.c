...
#include "imx709mipiraw_Sensor_23021.h"
#include "imx709_ana_gain_table_23021.h"
#include "imx709_eeprom_23021.h"
#include "imx709_vcm_23021.h"
...
#define PFX "imx709_camera_sensor_23021"
...
static struct oplus_get_eeprom_common_data imx709_common_data = {0};

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX709_SENSOR_ID_23021,
    ...
    .custom1 = { /* reg_H5 1632x1224 @10FPS */
        .pclk = 864000000,
        .linelength = 3700,
        .framelength = 23344,
        .startx = 0,
...
static void read_module_data(struct subdrv_ctx *ctx)
{
	...
	memset(&imx709_common_data, 0x00, sizeof(imx709_common_data));

	imx709_common_data.header[EEPROM_MODULE_ID] = 2;
	read_imx709_eeprom_info_23021(ctx, EEPROM_META_MODULE_ID,
				&(imx709_common_data.data[idx]), 2);
	imgsensor_info.module_id = (kal_uint16)(imx709_common_data.data[idx + 1] << 8) |
				   imx709_common_data.data[idx];
	idx += imx709_common_data.header[EEPROM_MODULE_ID];

	imx709_common_data.header[EEPROM_SENSOR_ID] = 2;
	read_imx709_eeprom_info_23021(ctx, EEPROM_META_SENSOR_ID,
				&(imx709_common_data.data[idx]), 2);
	idx += imx709_common_data.header[EEPROM_SENSOR_ID];

	imx709_common_data.header[EEPROM_LENS_ID] = 2;
	read_imx709_eeprom_info_23021(ctx, EEPROM_META_LENS_ID,
				&(imx709_common_data.data[idx]), 2);
	idx += imx709_common_data.header[EEPROM_LENS_ID];

	imx709_common_data.header[EEPROM_VCM_ID] = 2;
	read_imx709_eeprom_info_23021(ctx, EEPROM_META_VCM_ID,
				&(imx709_common_data.data[idx]), 2);
	idx += imx709_common_data.header[EEPROM_VCM_ID];

	imx709_common_data.header[EEPROM_MODULE_SN] = 23;
	read_imx709_eeprom_info_23021(ctx, EEPROM_META_MODULE_SN,
				&(imx709_common_data.data[idx]), 23);
	idx += imx709_common_data.header[EEPROM_MODULE_SN];

	imx709_common_data.header[EEPROM_AF_CODE_MACRO] = 2;
	imx709_common_data.header[EEPROM_AF_CODE_INFINITY] = 2;
	imx709_common_data.header[EEPROM_AF_CODE_MIDDLE] = 2;
	read_imx709_eeprom_info_23021(ctx, EEPROM_META_AF_CODE,
				&(imx709_common_data.data[idx]), 6);
    ...
    for (idx = 0; idx < 40; idx = idx + 4)
        LOG_INF("In %s: cam data: %02x %02x %02x %02x\n", __func__,
               imx709_common_data.data[idx], imx709_common_data.data[idx + 1],
               imx709_common_data.data[idx + 2],
               imx709_common_data.data[idx + 3]);
}
...
static u16 imx709_preview_setting[] = {
...
/* 3264x2448 @29FPS w/ PD  (On the fly with N-3) */
static u16 imx709_capture_setting[] = {
    //N-2 3264x2448 @29FPS w/ PD  (On the fly with N-3)
    //MIPI output setting
    0x0112, 0x0A,
    0x0113, 0x0A,
...
/* reg_I5_1 3264x1856 @30FPS ( Dbin 2-DOL, 1118M ) max VB 1ST */
static u16 imx709_custom6_setting[] = {
    //MIPI output setting
    0x0112, 0x0A,
    0x0113, 0x0A,
    0x0114, 0x03,
...
static u16 imx709_custom7_setting[] = {
//4Lane
//reg_N-4
...
static u16 imx709_custom9_setting[] = {
//4Lane
//reg_N-3
...
static u16 imx709_custom10_setting[] = {
/* reg_H3 1632x1224 @30FPS w/ PD PCLK=355.2M */
//4Lane
    //MIPI output setting
    0x0112, 0x0A,
...
static kal_uint16 imx709_custom11_setting[] = {
//4Lane
//reg_N-4
//3264x2448 @24FPS w/ PD
//H: 3264
...
static kal_uint16 imx709_custom12_setting[] = {
//4Lane
//reg_N-3-LLP
...
static kal_uint16 imx709_custom13_setting[] = {
//4Lane
//reg_N-2
...
{
	LOG_INF("%s start\n", __func__);
	imx709_table_write_cmos_sensor(ctx, imx709_custom11_setting,
			sizeof(imx709_custom11_setting)/sizeof(kal_uint16));
	LOG_INF("%s end\n", __func__);
}
...
{
	LOG_INF("%s start\n", __func__);
	imx709_table_write_cmos_sensor(ctx, imx709_custom12_setting,
			sizeof(imx709_custom12_setting)/sizeof(kal_uint16));
	LOG_INF("%s end\n", __func__);
}
...
{
	LOG_INF("%s start\n", __func__);
	imx709_table_write_cmos_sensor(ctx, imx709_custom13_setting,
			sizeof(imx709_custom13_setting)/sizeof(kal_uint16));
	LOG_INF("%s end\n", __func__);
}
/*
	...
	kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
	...
	ret = adaptor_i2c_wr_p8(ctx->i2c_client, IMX709_EEPROM_SLAVE_ADDRESS >> 1,
...
{
	kal_int32 ret = ERROR_NONE;
	kal_uint16 reg = IMX709_EEPROM_LOCK_REGISTER_23021;
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, IMX709_EEPROM_SLAVE_ADDRESS >> 1,
		reg, (IMX709_EEPROM_SLAVE_ADDRESS & 0xFE) | 0x01);
	} else {
		adaptor_i2c_wr_u8(ctx->i2c_client, IMX709_EEPROM_SLAVE_ADDRESS >> 1,
		reg, IMX709_EEPROM_SLAVE_ADDRESS & 0xFE);
	}
            ...
            ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == IMX709_SENSOR_ID_23021) && (data_length == CALI_DATA_SLAVE_LENGTH)
			&& (data_base == IMX709_STEREO_START_ADDR_23021)) {
			...
			write_eeprom_protect(ctx, 1);
			msleep(6);
			LOG_INF("com_0:0x%x\n", read_cmos_eeprom_8(ctx, IMX709_STEREO_START_ADDR_23021));
			LOG_INF("com_39:0x%x\n", read_cmos_eeprom_8(ctx, IMX709_STEREO_START_ADDR_23021+39));
			LOG_INF("innal_40:0x%x\n", read_cmos_eeprom_8(ctx, IMX709_STEREO_START_ADDR_23021+40));
			LOG_INF("innal_1556:0x%x\n", read_cmos_eeprom_8(ctx, IMX709_STEREO_START_ADDR_23021+1544));
			LOG_INF("write_Module_data Write end\n");
		} else if ((pStereodata->uSensorId == IMX709_SENSOR_ID_23021) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == IMX709_AESYNC_START_ADDR_23021)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                    read_cmos_eeprom_8(ctx, IMX709_AESYNC_START_ADDR_23021),
                    read_cmos_eeprom_8(ctx, IMX709_AESYNC_START_ADDR_23021+1),
                    read_cmos_eeprom_8(ctx, IMX709_AESYNC_START_ADDR_23021+2),
                    read_cmos_eeprom_8(ctx, IMX709_AESYNC_START_ADDR_23021+3),
                    read_cmos_eeprom_8(ctx, IMX709_AESYNC_START_ADDR_23021+4),
                    read_cmos_eeprom_8(ctx, IMX709_AESYNC_START_ADDR_23021+5),
                    read_cmos_eeprom_8(ctx, IMX709_AESYNC_START_ADDR_23021+6),
                    read_cmos_eeprom_8(ctx, IMX709_AESYNC_START_ADDR_23021+7));
            LOG_INF("AESync write_Module_data Write end\n");
        }else {
            LOG_INF("imx709 write_Module_data pStereodata is null\n");
		    return -1;
		}
	}
	return ret;
            ...
            if (*sensor_id == IMX709_SENSOR_ID) {
                *sensor_id = imgsensor_info.sensor_id;
                ...
                if(first_read){
                    ...
                    read_imx709_LRC_23021(ctx, imx709_LRC_setting);
                    read_imx709_QSC_23021(ctx, imx709_QSC_setting);
...
static int open(struct subdrv_ctx *ctx)
{
    ...
    if (ctx->is_esd_enable == true) {
        ...
        write_imx709_vcm_23021_init(ctx);
        ...
        enum SENSOR_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
	...
	{
		unsigned short imx709_seamless_preview[] = {
			0x0342, 0x2D,
			0x0343, 0xF0,
			0x0346, 0x00,
			0x0347, 0x10,
		...
		FMC_GPH_START;
		imx709_table_write_cmos_sensor(ctx, imx709_seamless_preview,
				sizeof(imx709_seamless_preview) / sizeof(kal_uint16));
		if (ae_ctrl) {
	...
	{
		unsigned short imx709_seamless_custom13[] = {
			0x0342, 0x2D,
			0x0343, 0xF0,
			0x0340, 0x09,
			0x0341, 0xE0,
		...
		FMC_GPH_START;
		imx709_table_write_cmos_sensor(ctx, imx709_seamless_custom13,
				sizeof(imx709_seamless_custom13) / sizeof(kal_uint16));
		if (ae_ctrl) {
	...
	{
		unsigned short imx709_seamless_custom12[] = {
			0x0342, 0x2E,
			0x0343, 0x08,
			0x0346, 0x04,
			0x0347, 0xD8,
		...
		FMC_GPH_START;
		imx709_table_write_cmos_sensor(ctx, imx709_seamless_custom12,
				sizeof(imx709_seamless_custom12) / sizeof(kal_uint16));
		if (ae_ctrl) {
...
//    int ret = ERROR_NONE;
    ...
    case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
        if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
            *(feature_data + 0) =
                sizeof(imx709_ana_gain_table_23021);
        } else {
            memcpy((void *)(uintptr_t) (*(feature_data + 1)),
            (void *)imx709_ana_gain_table_23021,
            sizeof(imx709_ana_gain_table_23021));
        }
        break;
...
//        break;
    ...
    *feature_para_len = 4;
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        if(*feature_para_len == sizeof(imx709_common_data) && feature_return_para_32 != feature_para_len) {
            memcpy(feature_return_para_32, &imx709_common_data,
                sizeof(imx709_common_data));
        }
        break;
...
//        LOG_INF("SENSOR_FEATURE_GET_EEPROM_STEREODATA  type:%d\n", *feature_data_16);
//        switch (*feature_data_16) {
//        case EEPROM_STEREODATA_MT_MAIN:
//        case EEPROM_STEREODATA_MW_MAIN:
//            read_imx709_eeprom_info_23021(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
...
#define EEPROM_8k 0x8000
    case SENSOR_FEATURE_GET_SENSOR_OTP_ALL:
        ...
        read_imx709_eeprom_23021(ctx, 0x00, (BYTE *)feature_return_para_32, *feature_para_len);
    break;
...
static struct mtk_mbus_frame_desc_entry frame_desc_cus11[] = { //reg_N-3_3264x2448 @29FPS Full-Crop Izoom w N-2
	{
		.bus.csi2 = {
			.channel = 0,
			.data_type = 0x2b,
...
const struct subdrv_entry imx709_mipi_raw_23021_entry = {
    .name = "imx709_mipi_raw_23021",
    .id = IMX709_SENSOR_ID_23021,
    .pw_seq = pw_seq,
