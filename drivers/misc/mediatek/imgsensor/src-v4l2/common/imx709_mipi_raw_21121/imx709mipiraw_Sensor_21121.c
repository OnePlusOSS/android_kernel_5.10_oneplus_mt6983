...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX709_SENSOR_ID_21121,
    ...
    .normal_video = { /* reg_C4-3-1 3264*1856@30fps*/
        .pclk = 864000000,
        .linelength = 11760,
        .framelength = 2448,
        .startx = 0,
    ...
    .custom6 = {/* reg_C4-3-1 3264*1856@30fps*/
        .pclk = 864000000,
        .linelength = 11760,
        .framelength = 2448,
        .startx = 0,
    ...
    .custom8 = { /*preisp reg_B3 3264*2448@30fps, reserved sensor mode*/
        .pclk = 720000000,
        .linelength = 7400,
        .framelength = 3240,
        .startx = 0,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[] = {
    ...
    {6560, 4928, 0,  16, 6560, 4896, 3280, 2448,  8,   0, 3264, 2448,  0,  0, 3264, 2448}, /* pre isp Previe8  reg_B3*/
...
{
    kal_uint16 idx = 0;
    read_imx709_eeprom_info_21121(ctx, EEPROM_META_MODULE_ID,
                &(imx709_common_data[0]), 2);
    imgsensor_info.module_id = (kal_uint16)(imx709_common_data[1] << 8) |
                   imx709_common_data[0];
    read_imx709_eeprom_info_21121(ctx, EEPROM_META_SENSOR_ID,
                &(imx709_common_data[2]), 2);
    read_imx709_eeprom_info_21121(ctx, EEPROM_META_LENS_ID,
                &(imx709_common_data[4]), 2);
    read_imx709_eeprom_info_21121(ctx, EEPROM_META_VCM_ID,
                &(imx709_common_data[6]), 2);
    read_imx709_eeprom_info_21121(ctx, EEPROM_META_MODULE_SN,
...
static void hdr_write_double_shutter(struct subdrv_ctx *ctx, kal_uint16 le, kal_uint16 se, kal_bool gph)
{
    kal_uint16 realtime_fps = 0;
    kal_uint16 exposure_cnt = 0;
    int i = 0;
    //kal_uint16 le_b = 0, se_b = 0;
    //LOG_INF("[test] 0x3247(0x%x)\n", read_cmos_sensor_8(ctx, 0x3247));
    //LOG_INF("[test] 0x30a0(0x%x)\n", read_cmos_sensor_8(ctx, 0x30a0));
    //LOG_INF("[test] 0x0e00(0x%x)\n", read_cmos_sensor_8(ctx, 0x0e00));
    //LOG_INF("[test] 0x0340 0x0341(%d)\n", ((read_cmos_sensor_8(ctx, 0x0340) &0xff) << 8) | (read_cmos_sensor_8(ctx, 0x0341) &0xff) );
    if (le)
        exposure_cnt++;
    if (se)
        exposure_cnt++;
    ...
    ctx->frame_length =
	    max((kal_uint32)(le + se + (exposure_cnt * imgsensor_info.margin)) * exposure_cnt,
	    ctx->min_frame_length);
    ctx->frame_length = min(ctx->frame_length, imgsensor_info.max_frame_length);
    LOG_INF("E! le:0x%x, se:0x%x autoflicker_en %d frame_length %d, gph %d\n",
        le, se, ctx->autoflicker_en, ctx->frame_length, gph);
    ...
    if (ctx->autoflicker_en) {
        ...
        if (realtime_fps >= 297 && realtime_fps <= 305)
            ...
            write_frame_len(ctx, ctx->frame_length, exposure_cnt);
        }
    } else {
        write_frame_len(ctx, ctx->frame_length, exposure_cnt);
    }
...
static void hdr_write_double_gain(struct subdrv_ctx *ctx, kal_uint16 lgain, kal_uint16 sgain, kal_bool gph)
{
    ...
    if (lgain < BASEGAIN || lgain > 64 * BASEGAIN) {
        ...
        if (lgain < BASEGAIN)
            ...
            lgain = (kal_uint16)(64 * BASEGAIN);
    }

    reg_lg = gain2reg(ctx, lgain);
    reg_sg = gain2reg(ctx, sgain);
    ctx->gain = reg_lg;
    if (gph) {
        write_cmos_sensor_8(ctx, 0x0104, 0x01);
    }
            ...
            if (*sensor_id == IMX709_SENSOR_ID) {
                *sensor_id = imgsensor_info.sensor_id;
                ...
                if(first_read){
                    ...
                    read_imx709_LRC_21121(ctx, imx709_LRC_setting);
                    read_imx709_QSC_21121(ctx, imx709_QSC_setting);
        ...
        enum SENSOR_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
    ...
    case SENSOR_SCENARIO_ID_CUSTOM7:
    {
        ...
        if (ae_ctrl) {
            ...
            hdr_write_double_shutter(ctx,
                    ae_ctrl[SHUTTER_LE_FRM_1],
                    ae_ctrl[SHUTTER_ME_FRM_1], KAL_FALSE);
            hdr_write_double_gain(ctx,
                    ae_ctrl[GAIN_LE_FRM_1],
...
static int
get_resolution(struct subdrv_ctx *ctx, MSDK_SENSOR_RESOLUTION_INFO_STRUCT *sensor_resolution)
{
    int i = 0;
    LOG_INF("E\n");
                 ...
                 UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
        if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
            *(feature_data + 0) =
                sizeof(imx709_ana_gain_table_21121);
        } else {
            memcpy((void *)(uintptr_t) (*(feature_data + 1)),
            (void *)imx709_ana_gain_table_21121,
            sizeof(imx709_ana_gain_table_21121));
        }
        break;
    ...
    case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
        *(MINT32 *)(signed long)(*(feature_data + 1)) = 6161000;
        LOG_INF("exporsure");
    break;
    ...
    *feature_para_len = OPLUS_CAMERA_COMMON_DATA_LENGTH;
    ...
    case SENSOR_FEATURE_GET_STAGGER_TARGET_SCENARIO:
        if (*feature_data == SENSOR_SCENARIO_ID_NORMAL_VIDEO) {
            ...
            switch (*(feature_data + 1)) {
            case HDR_NONE:
                *(feature_data + 2) = SENSOR_SCENARIO_ID_NORMAL_VIDEO;//normal_video mode for video preview mode
                break;
            default:
                break;
            }
    ...
    case SENSOR_FEATURE_SET_HDR_SHUTTER://for 2EXP
        ...
        hdr_write_double_shutter(ctx, (UINT16)*feature_data,
            (UINT16)*(feature_data+1), KAL_TRUE);
        break;
    case SENSOR_FEATURE_SET_DUAL_GAIN://for 2EXP
        LOG_INF("SENSOR_FEATURE_SET_DUAL_GAIN LE=%d, SE=%d\n",
            (UINT16) *feature_data, (UINT16) *(feature_data + 1));
        // implement write gain for NE/SE
        hdr_write_double_gain(ctx, (UINT16)*feature_data,
            (UINT16)*(feature_data+1), KAL_TRUE);
        break;
...
const struct subdrv_entry imx709_mipi_raw_21121_entry = {
    .name = "imx709_mipi_raw_21121",
    .id = IMX709_SENSOR_ID_21121,
    .pw_seq = pw_seq,
