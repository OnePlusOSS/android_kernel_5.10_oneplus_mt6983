...
static inline bool is_systemserver_process(struct task_struct *t) {
	if (!strcmp(t->comm, "system_server") || !strncmp(t->group_leader->comm, "Binder:", 7)) {
		return true;
	} else {
		return false;
	}
...
static inline bool is_critial_process(struct task_struct *t) {
	if (t->group_leader && (!strcmp(t->group_leader->comm, "system_server") || !strncmp(t->group_leader->comm, "Binder:", 7)
