...
static char aw8697_rtp_name_178Hz[][AW8697_RTP_NAME_MAX] = {
	...
	{"aw8697_stepable_regulate_RTP_105_178Hz.bin"},
...
static int aw8697_parse_dt(struct device *dev, struct aw8697 *aw8697,
    ...
    val = of_property_read_u8_array(np, "aw8697_vmax", vmax, ARRAY_SIZE(vmax));
    if (val != 0) {
        dev_info(dev, "aw_haptic_vmax not found");
    } else {
        for (i = 0; i < ARRAY_SIZE(vmax); i++) {
            vmax_map[i].vmax = vmax[i];
            dev_info(dev, "vmax_map vmax: 0x%x vmax: 0x%x", vmax_map[i].vmax, vmax[i]);
        }
    }

    val = of_property_read_u8_array(np, "aw8697_gain", gain, ARRAY_SIZE(gain));
    if (val != 0) {
        dev_info(dev, "aw_haptic_gain not found");
    } else {
        for (i = 0; i < ARRAY_SIZE(gain); i++) {
            vmax_map[i].gain = gain[i];
            dev_info(dev, "vmax_map gain: 0x%x gain: 0x%x", vmax_map[i].gain, gain[i]);
        }
    }
    return 0;
}
